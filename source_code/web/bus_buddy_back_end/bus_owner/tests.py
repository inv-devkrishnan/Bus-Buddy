...
class BaseTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(username="testuser", password="password",role=3)
        self.bus = Bus.objects.create(
            bus_name="Test Bus",
            user_id=self.user, 
            plate_no="Test123",
...
class BusOwnerTest(BaseTest):
    ...
    def test_can_soft_delete_bus(self):
        self.delete_bus_url = reverse("delete-bus", args=[self.bus.id])
        response = self.client.put(self.delete_bus_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        # Check if the bus status is updated to 99 in the database
        updated_bus = Bus.objects.get(id=self.bus.id)
        self.assertEqual(updated_bus.status, 99)

        # You can add additional assertions for related objects if needed

    def test_soft_delete_already_deleted_bus(self):
        self.delete_bus_url = reverse("delete-bus", args=[self.bus.id])
        # Soft delete the bus first
        self.bus.status = 99
        self.bus.save()

        response = self.client.put(self.delete_bus_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["message"], "bus already deleted")

    def test_soft_delete_invalid_bus_id(self):
        self.delete_bus_url = reverse("delete-bus", args=[self.bus.id])
        # Assuming an invalid bus ID that doesn't exist
        invalid_delete_bus_url = reverse("delete-bus", args=[999])

        response = self.client.put(invalid_delete_bus_url)
